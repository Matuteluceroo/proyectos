...
export class ContenidoModel {
  ...
  // static async crear({ titulo, descripcion, id_tipo, id_usuario, almacenamiento, url_archivo, html, textoPlano }) {
  //   const request = new sql.Request()
  ...
  //   const result = await request.query(`SELECT TOP 1 * FROM ${tableName} ORDER BY id_contenido DESC`)
  //   return result.recordset[0]
  // }
  ...
  //   const request = new sql.Request()
  //   request.input('id', sql.Int, id)
  //   const result = await request.query(`SELECT * FROM ${tableName} WHERE id_contenido = @id`)
  //   return result.recordset[0]
  // }
}
