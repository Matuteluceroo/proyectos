...
export class HistorialController {
  ...
  static async registrarConsulta(req, res) {
    try {
      const { id_usuario, id_contenido, tipo } = req.body; // tipo: 'HTML' | 'ARCHIVO'
      console.log("📥 registrarConsulta:", id_usuario, id_contenido, tipo);

      await HistorialModel.registrarConsulta({
        id_usuario,
        id_contenido,
        tipo,
      });
      return res
        .status(201)
        .json({ mensaje: "Consulta registrada correctamente" });
    } catch (error) {
      console.error("❌ Error en registrarConsulta:", error);
